...
int sum_card(int **score_card, int size) {
    // int **score_card --> score_card is a list of pointers 
    // so score_card[i] --> store a pointer
    // we only have to dereference once to get the value from score_card[i]
    // int *score_card --> a pointer to an array
